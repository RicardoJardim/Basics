...
exports.install = function() {
	...
	var db = Ti.Database.install('database_sql.sqlite', 'database_sql');
	db.close();
	
	var dbOpen = Ti.Database.open('database_sql');
	
	var sqlQuery = 'SELECT * FROM user';
	
	var sqlTemp = dbOpen.execute(sqlQuery);
		
		while (sqlTemp.isValidRow()) {
			
	       var id = sqlTemp.fieldByName('id');
	       var name = sqlTemp.fieldByName('name');
	       console.log(id + ' / ' + name);
	       sqlTemp.next();
		}
